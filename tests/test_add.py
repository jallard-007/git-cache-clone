...
def test_add_creates_cache(tmp_path):
    ...
    r = repos_dir / git.normalize_uri(str(repo_path)) / filenames.REPO_DIR
    assert r.is_dir()
...
def test_add_cache_already_exists_and_refreshes(tmp_path):
    ...
    pod_repo_dir = repos_dir / git.normalize_uri(str(repo_path)) / filenames.REPO_DIR
    assert pod_repo_dir.is_dir()
    ...
    result = core.add(config, str(repo_path), refresh_if_exists=True)
